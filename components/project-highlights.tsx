"use client"

import { useState, useEffect } from "react"
import { Shield, Wifi, Camera, Users, MapPin, CheckCircle, LucideSnowflake, Building2, Zap, Droplets, Users2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import { BookingModal } from "@/components/booking-modal"
import apiService from "@/lib/api-service"
import { ProjectHighlight } from "@/lib/website-data"



// Icon mapping
const iconMap: { [key: string]: any } = {
  Shield,
  Wifi,
  Camera,
  Users,
  MapPin,
  CheckCircle,
  LucideSnowflake,
  Building2,
  Zap,
  Droplets,
  Users2
}

export function ProjectHighlights() {
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [highlights, setHighlights] = useState<ProjectHighlight[]>([])
  const [guarantees, setGuarantees] = useState<any[]>([])
  const [trustIndicators, setTrustIndicators] = useState<any>(null)
  const [isLoading, setIsLoading] = useState(true)

  // Function to load trust indicators from API
  const loadTrustIndicators = async () => {
    try {
      const response = await fetch('/api/website-data/trust-indicators')
      if (response.ok) {
        const data = await response.json()
        if (data.success && data.data?.guarantees) {
          setGuarantees(data.data.guarantees)
          setTrustIndicators(data.data)
        }
      }
    } catch (error) {
      console.error('Error loading trust indicators:', error)
    }
  }

  // Function to manually refresh data
  const refreshData = async () => {
    setIsLoading(true)
    await loadTrustIndicators()
    setIsLoading(false)
  }

  useEffect(() => {
    // Load data from API
    const loadData = async () => {
      try {
        setIsLoading(true)
        const [highlightsData] = await Promise.all([
          apiService.getProjectHighlightsData(),
          loadTrustIndicators()
        ])
        
        setHighlights(highlightsData)
      } catch (error) {
        console.error('Error loading project highlights data:', error)
        // Fallback to default data
        setHighlights([
          { icon: "Shield", title: "بيئة سكنية متكاملة" },
          { icon: "LucideSnowflake", title: "مداخل مكيفة" },
          { icon: "Camera", title: "كاميرات مراقبة" },
          { icon: "Wifi", title: "سمارت هوم" }
        ])
        setGuarantees([
          { icon: "Building2", title: "ضمان الجودة", subtitle: "15 سنة ضمان على الهيكل الإنشائي" },
          { icon: "Zap", title: "ضمان الكهرباء", subtitle: "25 سنة ضمان على القواطع والأفياش" }
        ])
      } finally {
        setIsLoading(false)
      }
    }

    loadData()

    // Set up polling to check for updates every 5 seconds
    const interval = setInterval(loadTrustIndicators, 5000)

    return () => clearInterval(interval)
  }, [])

  // Filter out incomplete guarantees
  const validGuarantees = guarantees.filter(guarantee => 
    guarantee.title && 
    guarantee.title !== "ضمان جديد" && 
    guarantee.subtitle && 
    guarantee.subtitle.trim() !== ""
  )

  if (isLoading) {
    return (
      <section className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4 md:px-6">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#540f6b] mx-auto"></div>
            <p className="mt-4 text-gray-600">جاري تحميل البيانات...</p>
          </div>
        </div>
      </section>
    )
  }

  if (!highlights || highlights.length === 0) {
    return null
  }

  return (
    <section className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4 md:px-6">
        {/* Section Header */}
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-2xl md:text-2xl lg:text-4xl font-bold text-[#2c2c2c] mb-4">
            <span className="font-bold text-[#540f6b]">مميزات المشروع</span>
          </h2>
          <p className="text-base md:text-base text-[#6b7280] max-w-3xl mx-auto">
            مساحات تصل إلى 155 م² - موقع استثنائي قريب من الواجهة البحرية والمطار وأهم الشوارع الرئيسية
          </p>
        </div>

        <div className="grid grid-cols-2 gap-4 md:gap-6 lg:gap-8 mb-12">
          {highlights.map((highlight, index) => {
            const IconComponent = iconMap[highlight.icon]
            return (
            <div
              key={index}
              className="flex items-center justify-start gap-3 md:gap-4 bg-[#f5f3f0] rounded-2xl py-4 md:py-6 md:px-4"
            >
              <div className="flex-shrink-0 flex items-center justify-center bg-white rounded-full w-12 md:w-16 h-12 md:h-16 border border-[#e5e1dc] mx-2">
                {IconComponent && <IconComponent className="w-6 md:w-8 h-6 md:h-8  text-[#540f6b]" />}  
              </div>
              <h3 className="font-bold text-sm md:text-lg lg:text-xl text-[#2c2c2c] leading-tight">
                {highlight.title}
              </h3>
            </div>
          )})}
        </div>

        {/* Guarantees Section */}
        <div className="mt-16 md:mt-20">
          <div className="text-center mb-12 md:mb-16">
            <div className="flex items-center justify-center gap-4 mb-4">
              <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#2c2c2c]">
                <span className="font-bold text-[#540f6b]">ضمانات المشروع</span>
              </h2>
      
            </div>
            <p className="text-lg md:text-xl text-[#6b7280] max-w-3xl mx-auto">
              نلتزم بأعلى معايير الجودة ونقدم ضمانات شاملة لراحة بالك وثقتك
            </p>
          </div>

          {/* Guarantees */}
          {validGuarantees.length > 0 ? (
            <div className="bg-[#f5f3f0] rounded-2xl p-6 md:p-4 border border-[#e5e1dc]">
              <div className="grid grid-cols-2 gap-4 md:gap-6">
                {validGuarantees.map((guarantee, index) => {
                  const IconComponent = iconMap[guarantee.icon] || Shield
                  return (
                    <div key={index} className="flex items-center gap-2 md:gap-3">
                      <div className="flex-shrink-0 flex items-center justify-center bg-white rounded-full w-8 md:w-10 h-8 md:h-10 border border-[#e5e1dc]">
                        {IconComponent && <IconComponent className="w-4 md:w-5 h-4 md:h-5 text-[#540f6b]" />}
                      </div>
       
                      <span className="text-[#2c2c2c] text-sm md:text-base font-medium">{guarantee.title} {guarantee.subtitle}</span>
                    </div>
                  )
                })}
              </div>
          
            </div>
          ) : (
            <div className="bg-[#f5f3f0] rounded-2xl p-6 md:p-4 border border-[#e5e1dc] text-center">
              <p className="text-[#6b7280] text-sm">لا توجد ضمانات متاحة حالياً</p>
                     </div>
          )}
        </div>

        {/* Booking Button */}
        <div className="mt-16 md:mt-20 text-center ">
          <Button
            onClick={() => {
              const formElement = document.getElementById('booking-form')
              if (formElement) {
                formElement.scrollIntoView({ behavior: 'smooth', block: 'start' })
              }
            }}
            className="bg-[#540f6b] hover:bg-[#540f6b] text-white  px-8 py-6 rounded-[18px] text-lg font-bold transition-all duration-300 hover:shadow-lg hover:transform hover:-translate-y-1"
          >
            احجز وتملك الآن شقة العمر
          </Button>
        </div>
      </div>

      {/* Booking Modal */}
      <BookingModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </section>
  )
}
